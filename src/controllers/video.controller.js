...
const uploadVideo = asyncHandler(async (req, res) => {
    ...
    console.log("   ✓ Thumbnail file received:", req.files?.thumbnail?.[0]?.filename);
    ...
    console.log("   ➜ Video Duration:", `${duration} seconds (${Math.floor(duration / 60)}:${String(Math.floor(duration % 60)).padStart(2, '0')})`);
...
const getAllVideos = asyncHandler(async (req, res) => {
    ...
    if (!category && !tags && !owner && !search) {
        console.log("   ➜ No additional filters applied (showing all published videos)");
    }
    ...
    console.log(`   ➜ Sorting by: ${sortBy} (${sortOrder === 'asc' ? 'ascending' : 'descending'})`);
...
const getVideoById = asyncHandler(async (req, res) => {
    ...
    console.log("   ➜ Requested by:", req.user?.username || "Anonymous (Not authenticated)");
    ...
    console.log(`   ➜ Published Status: ${foundVideo.isPublished ? 'Public' : 'Private'}`);
    ...
    console.log(`   ✓ View count updated: ${foundVideo.views} → ${updatedVideo.views}`);
    ...
    console.log(`   ⏱️  Duration: ${Math.floor(updatedVideo.duration / 60)}:${String(Math.floor(updatedVideo.duration % 60)).padStart(2, '0')}`);
    console.log("=".repeat(60) + "\n");
...
const updateVideo = asyncHandler(async (req, res) => {
    ...
    if (description !== undefined) console.log(`      • Description: ${description ? `"${description.substring(0, 50)}${description.length > 50 ? '...' : ''}"` : '(empty)'}`);
    if (thumbnailFileLocalPath) console.log(`      • Thumbnail: ${req.file?.filename}`);
